...
| Attribute | Value |
| --- | --- |
| Module focus | Requirements, Specifications and Compliance |
| Area | Documentation, Reporting and Technical Writing |
| Operational purpose | This module develops the language needed to express requirements, define limits, state compliance positions, and write specification-style prose. |
| Primary documentary output | Requirement statements and compliance wording |
| Priority companion resources | [Requirement Template](../document-templates/requirement-template.md)<br>[Compliance and Standards Terms](../glossaries/compliance-and-standards-terms.md)<br>[Requirements and Compliance Models](../model-answers/requirements-and-compliance-models.md) |
...
This module develops the language needed to express requirements, define limits, state compliance positions, and write specification-style prose.

Requirement language is one of the most sensitive forms of technical English. Weak phrasing can create disagreement between design, quality, procurement, and integration teams. Precision is therefore essential.
...
The module emphasises *shall* for requirements, careful use of *must* and *should*, and phrases that define scope and exceptions, such as **unless otherwise specified** and **within the limits stated in**.
...
- writing requirements that are testable
- avoiding hidden ambiguity
- distinguishing compliance claim from evidence source
- stating exceptions without weakening the requirement
...
> The enclosure must be strong and the connector should still work after use.

### Stronger version

> The enclosure shall maintain interface integrity under nominal operating vibration levels. The front-access connector shall remain serviceable after ten installation and removal cycles.
...
A project note uses qualitative adjectives such as *strong*, *suitable*, and *good enough* in place of verifiable criteria. Rewrite the text as a set of requirement-style statements with clear compliance logic.
...
Draft five requirement statements and specify the intended verification method for each one.
...
Explain why some sentences describe design intent while others constitute actual requirements.
...
Create `requirements-writing-guide.md` with rule examples, testability notes, and common failure patterns.
...
A verification team must convert design intent into stable requirement statements and later link them to compliance evidence.

### Documentary risk

The main risk is unstable modality and under-specified acceptance wording. A page can look formal while remaining impossible to verify objectively.
...
A strong answer writes requirements that are specific, bounded, and verifiable. It distinguishes requirement from rationale, recommendation, and implementation note.
...
During correction, always ask: can this statement be verified? If the answer is no, the language still needs engineering discipline.
...
Create a requirements page that includes requirement syntax, bad-example critique, and compliance cross-reference logic.
...
This module should be studied together with [Requirement Template](../document-templates/requirement-template.md) and the related template or exercise pages. Premium progression depends on repeated transfer, not on single-pass reading.
