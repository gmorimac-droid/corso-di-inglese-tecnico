...
| Attribute | Value |
| --- | --- |
| Module focus | Describing Processes and Workflows |
| Area | Engineering Communication |
| Operational purpose | Engineering processes must be described in a way that makes order, dependency, and decision points visible. |
| Primary documentary output | Process or workflow description page |
| Priority companion resources | [Functional Language](../language-tools/functional-language.md)<br>[Phrasebank for Description](../language-tools/phrasebank-for-description.md)<br>[Writing Exercises](../exercises/writing-exercises.md) |
...
Engineering processes must be described in a way that makes order, dependency, and decision points visible. This module develops the language required for procedures, workflows, and process explanations.

When process language is weak, teams may perform steps in the wrong order, omit conditions, or misunderstand handover points between departments. Accurate workflow language therefore supports both quality and coordination.
...
Language work covers sequence markers, conditional structures, dependency wording, and verbs for process control such as **initiate**, **verify**, **release**, **record**, **escalate**, and **close**.
...
- showing order without monotony
- describing conditional branches
- writing processes for mixed audiences
- distinguishing procedural instruction from process explanation
...
> First you receive the unit and then you check it and if there is a problem you tell someone and if not you continue.

### Stronger version

> Upon receipt, the inspection record is opened and the unit condition is verified. Any visible damage triggers immediate escalation for technical review; otherwise, the workflow proceeds to functional verification.
...
A process description mixes chronological order with explanatory comments, making the sequence difficult to follow. Reorganise the text so that preconditions, actions, decision points, and outputs are clearly separated.
...
Write a controlled workflow description for a calibration, inspection, or issue-escalation process. Use at least three sequence markers and two condition statements.
...
Explain a workflow verbally to a new team member. Keep the explanation concise but make the decision points explicit.
...
Create `workflow-description-model.md` with stages, triggers, decision points, and closure conditions.
...
A quality-reviewed workflow must show inspection, decision points, escalation, and release steps for a maintenance-related process.

### Documentary risk

Process writing often becomes a chronological story with weak control of conditions, branches, and document references.
...
A strong answer distinguishes sequence from logic. It uses explicit connectors, embeds conditions where needed, and makes the point of each step visible to the reader.
...
When marking student work, circle verbs first. Process writing quality is often determined by whether the chosen verbs represent action, verification, approval, or state.
...
Create a Markdown workflow page with numbered phases, conditional notes, and a closing section on records generated.
