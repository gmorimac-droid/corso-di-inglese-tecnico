...
| Attribute | Value |
| --- | --- |
| Module focus | Reports, Summaries and Meeting Notes |
...
Reports and meeting records convert technical events into structured knowledge. This module teaches how to capture context, findings, decisions, and next actions in a disciplined way.

Without structured summaries, engineering work becomes harder to review and harder to resume. Good reports and minutes do not transcribe everything; they preserve what matters.
...
The module focuses on concise summary language, action-tracking phrases, and the difference between descriptive minutes and decision-oriented minutes. Learners also practise writing summaries that are shorter than the source material but more useful to a busy reviewer.
...
- distinguishing summary from detailed report
- capturing decisions without emotional language
- writing action items that are assignable and verifiable
- linking meeting outcomes to subsequent documentation
...
> In the meeting we talked about the enclosure and connector and there are still some things to check.

### Stronger version

> The review confirmed provisional acceptance of the enclosure concept. Final closure remains dependent on dimensional confirmation of connector clearance under the revised mounting geometry.
...
A meeting note includes discussion details but no actionable output. Convert the note into a usable record containing findings, decisions, open issues, and owners.
...
Prepare a one-page summary of a technical review meeting, including attendees, key decisions, open actions, and one unresolved risk.
...
Present a short verbal recap of a meeting in no more than ninety seconds, focusing only on outcomes and next actions.
...
Create `review-summary-model.md` and `action-log-example.md` for later reuse in project documentation.
...
A short meeting note must capture technical decisions, open points, responsibilities, and agreed follow-up after a design review.

### Documentary risk

Learners may record everything that was said instead of extracting only the decision-bearing and action-bearing information.
...
A premium answer groups information into purpose, findings, decisions, actions, and residual risks or assumptions. Brevity must not destroy accountability.
...
Ask students to identify the minimum viable record. Good notes are selective, not exhaustive.
...
Create repository-ready notes and summary pages with explicit action-item language and review-friendly formatting.
