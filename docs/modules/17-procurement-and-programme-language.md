...
| Attribute | Value |
| --- | --- |
| Module focus | Procurement and Programme Language |
...
Engineering communication at programme level must address deliverables, reviews, revisions, contractual expectations, and configuration discipline. This module introduces the associated language.

Technical teams often participate in procurement and programme communication even when they are not contract specialists. They therefore need a working command of terms related to deliverables, milestones, revision status, and compliance expectations.
...
The module uses formal phrases such as **subject to approval**, **for contractual review**, **to be delivered by**, **under the current baseline**, and **pending closure of open actions**.
...
- writing at programme rather than component level
- describing document maturity and review status
- referring to contractual or institutional expectations without legal overreach
- summarising progress in a controlled formal register
...
> The document is almost ready but there are still some pending checks before we can send it outside.

### Stronger version

> Revision 3 has been released for internal review; however, external issue remains pending closure of Action 07 and formal confirmation of the updated connector allocation table.
...
A status email confuses draft maturity, approval state, and contractual deliverable status. Rewrite it so that stakeholders can understand exactly what has been completed, what remains open, and what blocks release.
...
Draft a short programme update covering milestone status, open actions, document maturity, and next review point.
...
Summarise project status in a formal review meeting using milestone, baseline, and deliverable terminology correctly.
...
Create `programme-language-reference.md` with common patterns for reviews, baselines, and deliverables.
...
A programme support note must summarise deliverables, milestones, documentation status, and supplier dependencies for a review meeting.

### Documentary risk

Programme language becomes weak when timelines, responsibilities, and technical deliverables are described in soft or ambiguous terms.
...
A strong answer names the deliverable, its status, its dependency, and the consequence of delay or incomplete data. It is factual rather than dramatic.
...
Ask learners to separate schedule information from technical significance. Good programme writing integrates both, but it does not confuse them.
...
Create a programme language page with milestone phrases, status expressions, and supplier-facing formulations.
