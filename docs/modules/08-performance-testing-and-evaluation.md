...
| Attribute | Value |
| --- | --- |
| Module focus | Performance, Testing and Evaluation |
...
Engineering communication often requires the writer to describe how an item performs under defined conditions, how testing was conducted, and what conclusions can or cannot be drawn from the available evidence.

Performance language becomes weak when learners confuse observation with interpretation or omit the conditions under which a result was obtained. Professional reporting must state setup, limits, findings, and significance with discipline.
...
The module trains learners to separate measured observations from analysis, to use cautious evaluative phrasing, and to state limitations explicitly. Structures such as **under nominal conditions**, **within the specified range**, **the results indicate that**, and **the current data does not support** are central.
...
- describing test context before conclusions
- comparing nominal and observed behaviour
- reporting deviations without exaggeration
...
> The test went well and the unit worked correctly except for a small issue that was not serious.

### Stronger version

> The unit completed all functional cycles under nominal test conditions. A minor output variation was observed at the upper end of the range, but the deviation remained below the acceptance limit and did not affect functionality.
...
A short report concludes that the design is reliable after one limited-duration test. Review the wording and correct any overstatement so that the conclusion reflects the actual evidence only.
...
Draft a one-page test summary including objective, setup, observations, limitation, and provisional conclusion.
...
Present test results to a review board in two minutes. Distinguish observed data, interpretation, and recommended follow-up action.
...
Create `test-summary-model.md` and link it to the report template and performance glossary terms.
...
A short evaluation note must summarise test conditions, results, limitations, and next action after a controlled bench test.

### Documentary risk

Students frequently confuse result with interpretation and write conclusions that exceed available evidence.
...
A strong test-related answer states the test context, reports measured or observed outcomes, qualifies interpretation, and indicates whether further verification is needed.
...
Reward caution and proportionality. Overconfident conclusions are a more serious professional weakness than modest lexical limitations.
...
Add a results page linking the evaluation summary to a test-report template and a glossary of performance descriptors.
