...
| Attribute | Value |
| --- | --- |
| Module focus | Grammar for Technical Communication |
| Area | Core Technical English |
| Operational purpose | Technical grammar is not an academic ornament. |
| Primary documentary output | Grammar-to-documentation conversion notes |
| Priority companion resources | [Grammar Reference](../language-tools/grammar-reference.md)<br>[Common Mistakes for Italian Speakers](../language-tools/common-mistakes-for-italian-speakers.md)<br>[Grammar Exercises](../exercises/grammar-exercises.md) |
...
Technical grammar is not an academic ornament. It determines obligation, scope, sequence, and evidential focus. This module studies grammar as a precision tool for engineering discourse.

In requirements, procedures, and reports, the difference between *shall*, *should*, *must*, and *may* is functionally important. The same is true for passive voice, conditionals, and relative clauses. Weak grammatical choices often produce ambiguity even when vocabulary is correct.
...
Key grammar points include passive structures for process description, modal verbs for obligation and recommendation, first-condition patterns for procedures and troubleshooting, and relative clauses for precise identification of parts, signals, or requirements.
...
- writing requirements that are mandatory but not overstated
- reporting observations without unjustified certainty
- expressing sequence and dependency
- clarifying which element a condition applies to
...
> You have to inspect the connector and maybe clean it. It should not be bad and you can use it again if it is okay.

### Stronger version

> The connector shall be inspected prior to installation. Reuse is permitted only if no physical damage is observed and interface integrity remains within the specified acceptance criteria.
...
A draft requirement uses *must*, *can*, and *should* inconsistently. The engineering meaning is unclear and reviewers disagree about whether the statements are mandatory or advisory. Rephrase the requirement set using a stable modal logic and explain your choices.
...
Rewrite an informal maintenance instruction into a disciplined procedure using passive voice where appropriate, clear sequencing, and correct modal verbs.
...
Explain the difference between *shall*, *should*, and *may* to a classmate using examples from requirements, procedures, and recommendations.
...
Create `grammar-for-engineering-reference.md` with a modal-verb decision table and three worked examples.
...
A document reviewer identifies repeated misuse of *shall*, *should*, and *must* across installation, safety, and verification pages.

### Documentary risk

Incorrect modality creates not only linguistic weakness but also documentary ambiguity. A recommendation can accidentally read like a requirement, and a requirement can become too soft.
...
A strong grammar answer does not treat grammar as abstraction. It shows how passive voice, modality, condition, and clause structure affect interpretation, responsibility, and compliance.
...
Correct grammar through function. Do not only mark errors; explain what documentary consequence each error creates, especially when requirement language is involved.
...
Create a repository page comparing statement types such as fact, requirement, recommendation, limitation, warning, and conditional instruction.
