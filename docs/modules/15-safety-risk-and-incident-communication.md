...
| Attribute | Value |
| --- | --- |
| Module focus | Safety, Risk and Incident Communication |
...
Safety language must support hazard awareness, preventive action, and incident traceability. This module develops the English used for warnings, risk notes, incident summaries, and mitigation statements.

Poor safety communication can lead to incorrect assumptions about severity, conditions, or required protective measures. Technical English in this domain must remain direct, proportionate, and unambiguous.
...
Language patterns include **shall not be operated unless**, **may result in**, **to reduce the risk of**, **in the event of**, and **immediate escalation is required when**. The module also distinguishes warning, caution, and notice language.
...
- writing proportional safety language
- describing risk without alarmist wording
- summarising incidents with factual discipline
- linking mitigation to identified hazards
...
> Do not use the unit if the connector is open because it can be dangerous.

### Stronger version

> The unit shall not be energised while the front-access connector remains exposed. Operation in this condition may result in contamination ingress and interface degradation.
...
An incident summary focuses on blame rather than factual sequence and control measures. Rewrite it so that event chronology, contributing conditions, and mitigation actions become clear without unnecessary personal language.
...
Draft a short incident summary and a separate mitigation note for future prevention.
...
Explain the difference between a hazard statement, a risk statement, and a mitigation statement using one example from maintenance or testing.
...
Create `safety-communication-guide.md` with warning/caution models and an incident-summary structure.
...
A near-miss involving incorrect connector handling must be documented with neutral language, clear risk framing, and proportionate corrective action.

### Documentary risk

Safety texts often become either too dramatic or too vague. Both extremes reduce documentary usefulness.
...
A strong answer distinguishes event description, exposure path, immediate control, residual risk, and recommended preventive measure.
...
Discourage blame language. Safety communication should focus on condition, behaviour, consequence path, and control logic.
...
Create a safety page combining warnings, incident summary, and controlled mitigation language.
